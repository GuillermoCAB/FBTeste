...
exports.commentOnScream = (req, res) => {
  ...
  db.doc(`/screams/${req.params.screamId}`).get()
    ...
    .then(doc => {
      let comments = doc.data().commentCount + 1
      db.doc(`screams/${req.params.screamId}`).update({ commentCount: comments })
    })
    .then(() => {
  ...
  screamDoc.get()
    ...
    .then(data => {
      if (data.empty) {
        ...
        return res.status(400).json({ error: 'Scream alredy liked' })
      }
    })
    .catch(err => {
      console.error(err)
